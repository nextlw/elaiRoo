...
const B = "-",
		...
		const { separator: r, experimentalParseClassName: t } = e,
			...
			a = (s) => {
				...
				const x = u.length === 0 ? s : s.substring(m),
					...
					w = v ? x.substring(1) : x,
					b = y && y > m ? y - m : void 0
				return { modifiers: u, hasImportantModifier: v, baseClassName: w, maybePostfixModifierPosition: b }
			}
	...
	be = (e) => ({ cache: de(e.cacheSize), parseClassName: pe(e), ...se(e) }),
	ge = /\s+/,
	fe = (e, r) => {
		...
		for (let u = a.length - 1; u >= 0; u -= 1) {
			const g = a[u],
				{ modifiers: m, hasImportantModifier: y, baseClassName: x, maybePostfixModifierPosition: v } = t(g)
			let w = !!v,
	...
	Te = () => {
		...
		return {
			...
			classGroups: {
				...
				"max-w": [{ "max-w": [i, r, "none", "full", "min", "max", "fit", "prose", { screen: [A] }, A] }],
				...
				"bg-image": [{ bg: ["none", { "gradient-to": ["t", "tr", "r", "br", "b", "bl", "l", "tl"] }, Ie] }],
				"bg-color": [{ bg: [e] }],
